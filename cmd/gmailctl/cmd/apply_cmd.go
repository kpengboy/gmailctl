...
func init() {
	...
	applyCmd.PersistentFlags().StringVar(&applyColor, "color", "auto", "whether to enable color output (must be \"always\", \"auto\" or \"never\")")
}
...
func apply(path string, interactive, test bool) error {
	...
	useColor, err := reporting.ShouldUseColorDiff(applyColor, "color")
	if err != nil {
		return err
	}
